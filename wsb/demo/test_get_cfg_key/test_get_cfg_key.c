...
SM_VOID Inn_GetCfgSignPrivateKey()
{
    ...
    SM_UCHAR key_buf[4096] = {0};
    SM_UINT key_len = sizeof(key_buf);
    ...
    printf("导出配用签名私钥......");
    ret = g_pDllMang->m_pfn_SM_ExportPrivateKey(g_stHandle.hPipe, key_handle, NULL, NULL, key_buf, &key_len);
    if (ret != SM_ERR_FREE)
    {
        printf("失败(%s)!\n\n", g_pDllMang->m_pfn_SM_GetErrorString(ret, FALSE));
        return FALSE;
    ...
    DumpHex(key_buf, key_len);
}



SM_VOID Inn_GetCfgVerifyPublicKey()
{
    SM_BLOB_KEY blob_key;
    SM_UINT     key_type = SMCK_ECC_VERIFY_PUBLIC;
    SM_KEY_HANDLE key_handle = SM_NULL;
    ...
    printf("导出配用验证公钥......");
    ret = g_pDllMang->m_pfn_SM_ExportPublicKey(g_stHandle.hPipe, key_handle, key_buf, &key_len);
    if (ret != SM_ERR_FREE)
    {
        printf("失败(%s)!\n\n", g_pDllMang->m_pfn_SM_GetErrorString(ret, FALSE));
        return FALSE;
...
SM_INT main()
{
    ...
    /* 备份配用密钥 */
    Inn_GetCfgSignPrivateKey();
    Inn_GetCfgVerifyPublicKey();
